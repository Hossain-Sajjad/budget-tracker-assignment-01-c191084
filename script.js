...
function calculateBudget() {
  availableBudget.innerHTML = parseFloat(totalIncome.innerHTML.replace(/,/g, "")) - parseFloat(totalExpense.innerHTML.replace(/,/g, ""));
}
calculateBudget();
...
function deleteEntry() {
  for (let [index, item] of deleteItem.entries()) {
...
function addEntry() {
  ...
  const newEntryHtml = `
    <li class="py-2.5 item-list">
      <div class="group flex justify-between gap-2 text-sm">
        <span>${description}</span>
        <div>
          <span class="${colorClass}">${sign}${formatMoney(value)}</span>
          <span
            class="ml-2 hidden cursor-pointer font-medium text-red-500 group-hover:inline-block delete-item"
          >
            Delete
          </span>
        </div>
